...
function FormTemplate({listId} : {listId: string}) {
   ...
   const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (selectedFile) {
         ...
         try {
            ...
            const response = await fetch(`${API_HOST}/lista-contactos/${listId}/from-file`, {
               method: "PUT",
               body: formData,
               headers: {
            ...
            if (!response.ok) {
               ...
               router.refresh();
               router.push(`/clientes/correo/lista-contactos/${listId}`);
            }            
         } catch (error) {
            setRequest({
               ...request,
   ...
   return (
      <form onSubmit={handleSubmit}>
         ...
         <span className="text-xs text-gray-500 text-right">
            Si un contacto ya se encuentra entre sus contactos, el mismo no será añadido. 
            Para añadirlo a lista deberá seleccionarlo en <Link className="underline text-blue-500" href={`/clientes/correo/lista-contactos/${listId}/anadir-contactos`}>Añadir contactos creados a lista</Link>
         </span>
      </form>
   );
}
